<?php
    ...
    $newArrivalsCategoryId = null;
    foreach ($dbCategories as $category) {
        if ($category['name'] === 'New Arrivals') {
            $newArrivalsCategoryId = $category['id'];
            break;
        }
    }
    ...
    if ($newArrivalsCategoryId) {
        $products = $productController->getProductsByCategory($newArrivalsCategoryId);
...
?>
    ...
    <div class="container">
            ...
            <section id="products" class="products">
                ...
                <h2 class="fw-bold display-7 mb-4 text-center">New Arrivals Products</h2>
                ...
                <form id="checkoutForm" method="POST" action="../../../app/controllers/OrderController.php">
                    ...
                    <div class="modal-body">
                        <div class="checkout-summary mb-4">
                            ...
                            <div id="checkoutItems" class="border rounded p-3 bg-light" style="max-height: 300px; overflow-y: auto;"></div>
                            <hr>
                        ...
                        <div class="card mb-3">
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    <label class="form-label fw-bold"><i class="fas fa-user me-1 text-muted"></i>Full Name</label>
                                    <input type="text" class="form-control bg-light" name="full_name" value="<?= htmlspecialchars($user['username'] ?? '') ?>" readonly>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label fw-bold"><i class="fas fa-envelope me-1 text-muted"></i>Email Address</label>
                                    <input type="email" class="form-control bg-light" name="email" value="<?= htmlspecialchars($user['email'] ?? '') ?>" readonly>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label fw-bold"><i class="fas fa-map-marker-alt me-1 text-muted"></i>Shipping Address</label>
                                    <textarea class="form-control bg-light" name="shipping_address" rows="3" readonly><?= htmlspecialchars($user['address'] ?? '') ?></textarea>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="payment-method-card" data-payment="paypal">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="payment-icon" style="color: #0070ba;"><i class="fab fa-paypal"></i></div>
                                        <div>
                    ...
                    <div class="modal-footer bg-light">
                        <div class="w-100">
                            <div class="row align-items-center">
                                <div class="col-md-6">
                                    <div class="small text-muted mb-1"><i class="fas fa-shield-alt me-1"></i>Secure checkout process</div>
                                    <div class="small text-muted"><i class="fas fa-truck me-1"></i>Free delivery available</div>
                                </div>
                ...
                <div class="modal-body bg-white">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="mt-3" id="modalAltImages"></div>
                        </div>
                        <div class="col-md-6">
                            ...
                            <div class="mb-4">
                                ...
                                <div id="modalProductSizes" class="mt-2"></div>
                            </div>
    ...
    <script src="../../../public/js/new-arrivals.js"></script>
    ...
    <script>
        ...
        function showPayPalResultModal(success, message) {
            ...
            if (!el) {
                ...
                wrapper.innerHTML = `<div class="modal fade" id="${modalId}" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content border-0 shadow-lg"><div class="modal-header border-0"><h5 class="modal-title fw-bold" id="${modalId}Title"></h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body text-center px-4 pb-4"><div id="${modalId}Icon" class="display-3 mb-3"></div><p id="${modalId}Msg" class="text-muted"></p></div><div class="modal-footer border-0 justify-content-center"><button class="btn btn-dark px-4" data-bs-dismiss="modal">OK</button></div></div></div></div>`;
            ...
            el.querySelector(`#${modalId}Icon`).innerHTML = success ? '<i class="fas fa-check-circle text-success"></i>' : '<i class="fas fa-times-circle text-danger"></i>';
        ...
        paypal.Buttons({
            createOrder: function (data, actions) {
                ...
                return actions.order.create({
                    purchase_units: [{ description: 'Empire Streetwear Purchase',
                        amount: { currency_code: 'PHP', value: payload.total.toFixed(2),
                            breakdown: { item_total: { currency_code: 'PHP', value: payload.total.toFixed(2) } } },
                        items: orderItems }],
                    application_context: { brand_name: 'EMPIRE STREETWEAR', user_action: 'PAY_NOW' }
                });
            },
            onApprove: function (data, actions) {
                return actions.order.capture().then(function (details) {
                    const payerName = (details.payer && details.payer.name)
                        ? details.payer.name.given_name + ' ' + (details.payer.name.surname || '') : 'Customer';
                    const form = document.getElementById('checkoutForm');
                    const address = form.querySelector('[name="shipping_address"]').value;
                    const payload = buildPayPalPayload();
                    if (!payload) { showPayPalResultModal(false, 'Cart data lost after payment. Contact support with PayPal ID: ' + data.orderID); return; }
                    ...
                    fetch('../../../app/controllers/OrderController.php?action=placeOrder', { method: 'POST', body: formData })
                    .then(r => r.json()).then(result => {
                        if (result.success) {
                            if (window.cartManager) { window.cartManager.cart = []; window.cartManager.saveCartToStorage(); window.cartManager.updateCartDisplay(); }
                            localStorage.removeItem('shopping_cart'); localStorage.removeItem('cart');
                            const checkoutModal = bootstrap.Modal.getInstance(document.getElementById('checkoutModal'));
                            if (checkoutModal) checkoutModal.hide();
                            showPayPalResultModal(true, 'Thank you, ' + payerName + '! Your order ' + (result.order_id ? '#' + result.order_id : '') + ' has been placed.');
                            document.getElementById('paypalResultModal').addEventListener('hidden.bs.modal', () => window.location.reload(), { once: true });
                        } else {
                            showPayPalResultModal(false, 'Payment received but order save failed: ' + (result.message || 'Unknown error. Contact support.'));
                        }
                    }).catch(err => { console.error('Order save error:', err); showPayPalResultModal(false, 'Payment received but order not saved. Contact support with PayPal order ID: ' + data.orderID); });
                });
            },
            onCancel: function () { showPayPalResultModal(false, 'Payment cancelled. Your cart is still intact.'); },
            onError: function (err) { console.error('PayPal error:', err); showPayPalResultModal(false, 'A PayPal error occurred. Please try again or choose Cash on Delivery.'); }
