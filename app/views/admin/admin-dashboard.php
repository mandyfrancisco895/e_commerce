...
if (isset($_SESSION['user_id'])) {
    ...
    if ($currentUser) {
        ...
        $_SESSION['role'] = $currentUser['role'] ?? 'admin';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_management_role'])) {
    ...
    try {
        ...
        // Log to maintenance_logs (audit trail)
        $actionMessage = "Created new " . ucfirst($role) . " account: $username";
        $stmtLog = $db->prepare("INSERT INTO maintenance_logs (admin_name, action_performed, status) VALUES (?, ?, 'Success')");
        $stmtLog->execute([$admin_name, $actionMessage]);

        $db->commit();
        
        $_SESSION['success_message'] = "Management account created successfully! Username: $username, Role: " . ucfirst($role);
        header("Location: admin-dashboard.php#rbac");
        exit;

    } catch (Exception $e) {
        $db->rollBack();
        error_log("Create management role error: " . $e->getMessage());
        $_SESSION['error_message'] = "Failed to create account: " . $e->getMessage();
        header("Location: admin-dashboard.php#rbac");
        exit;
    }
}
...
// ✅ FIXED: Include profile_pic column
$userStmt = $db->prepare("SELECT id, username, email, role, status, profile_pic FROM users ORDER BY id DESC");
$userStmt->execute();
$allUsers = $userStmt->fetchAll(PDO::FETCH_ASSOC);
