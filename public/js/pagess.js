...
showCartNotification(productName, quantity = 1) {
    ...
    initializeEventListeners() {
        const self = this; // Store reference to CartManager instance
        
        // Add to cart buttons in product grid ONLY (not in modal)
        document.addEventListener('click', (e) => {
            // Make sure we're NOT in the product modal
            if (e.target.closest('#productDetailsModal')) {
                return; // Skip if inside modal
            }
        ...
        // FIXED: Add to cart button in modal ONLY - Prevent double add
        let modalAddToCartProcessing = false;
        
        document.addEventListener('click', (e) => {
            // Only process if we're inside the product modal
            if (!e.target.closest('#productDetailsModal')) {
                return; // Skip if not in modal
            }
            
            // Check if it's the Add to Cart button
            const isModalAddBtn = e.target.classList.contains('btn-dark') && 
                                  e.target.classList.contains('btn-lg') &&
            ...
            if (isModalAddBtn) {
                ...
                if (modalAddToCartProcessing) {
                    console.log('Add to cart already processing, ignoring duplicate click');
                    return;
                }
                modalAddToCartProcessing = true;
                ...
                console.log('Adding product with size:', selectedSize);
                
                if (self.addToCart(productData, selectedSize)) {
                ...
                setTimeout(() => {
                    modalAddToCartProcessing = false;
                }, 1000);
            }
        }, true); // Use capture phase to catch event first
    ...
    if (productModalElement) {
        ...
        clickableImages.forEach(image => {
            image.addEventListener('click', function() {
                ...
                const addToCartBtn = document.querySelector('#productDetailsModal .btn-dark.btn-lg');
                ...
                document.getElementById('modalAltImages').innerHTML = '';
